...
B12X source builds now use `local-inference-lab/vllm@dev/infernal-incantation`.
...
Added `--exp-b12x` (alias: `--experimental-b12x`) as an alternative version built from [a fork by Luke Alonso](https://github.com/local-inference-lab/vllm/tree/dev/infernal-incantation). This fork supports a collection of experimental high-performance B12X kernels for sm12x architecture.
...
from `local-inference-lab/vllm@dev/infernal-incantation` and the `master` branch of the
B12X repository, run:
