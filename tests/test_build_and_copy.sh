...
test_exp_b12x_rebuild_vllm_uses_preset_source_build() {
    ...
    assert_output_contains 'Building B12X from https://github\.com/lukealonso/b12x\.git ref master for https://github\.com/local-inference-lab/vllm ref dev/infernal-incantation\.'
...
test_exp_b12x_allows_vllm_prs() {
    ...
    assert_log_contains '^docker build --target vllm-export .*--build-arg VLLM_REF=dev/infernal-incantation --build-arg VLLM_REPO=https://github.com/local-inference-lab/vllm --build-arg VLLM_APPLY_PRESET_PRS=0 --build-arg CACHEBUST_VLLM=[0-9]+ --build-arg VLLM_PRS=12345'
